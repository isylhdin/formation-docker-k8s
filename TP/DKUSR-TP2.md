...
Veillez à quitter le conteneur dans lequel vous êtes. Puis préparez à travailler dans le répertoire de travail `~/luca/kubernetes-app/` (le répertoire est déjà présent). Nous allons être amenés à créer plusieurs fichiers dans ce répertoire :
...
Maintenant que nous avons créé notre fichier Dockerfile, il est temps de créer notre fichier docker-compose qui va nous simplifier la construction et le lancement de notre application. Nous allons créer un fichier `docker-compose.yml` (toujours dans  `~/luca/kubernetes-app/`) avec le contenu suivant :
```yaml
...
Successfully tagged 261407191094.dkr.ecr.eu-west-1.amazonaws.com/luca/app:v0.1
```
